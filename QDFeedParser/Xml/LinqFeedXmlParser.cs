...
namespace QDFeedParser.Xml
{
	public class LinqFeedXmlParser : FeedXmlParserBase
	{
		...
		public override FeedType CheckFeedType(string feedxml)
		{
			...
			if (xmlRootElement.Name.LocalName.Contains(RssRootElementName) && xmlRootElement.Attribute(RssVersionAttributeName).Value == "0.92")
			{
		...
		protected virtual void ParseRss092Header(Rss092Feed rssFeed, string xml)
		{
			var document = XDocument.Parse(xml);
			var channel = document.Root.Element("channel");
		...
		private void ParseRss092Items(Rss092Feed rssFeed, string xml)
		{
			var document = XDocument.Parse(xml);
			var feedItemNodes = document.Root.Element("channel").Elements("item");
			foreach (var item in feedItemNodes)
			{
				rssFeed.Items.Add(ParseRss092SingleItem(item));
			}
		}

		protected virtual Rss092FeedItem CreateRss092FeedItem()
		{
			return new Rss092FeedItem();
		}

		protected virtual BaseFeedItem ParseRss092SingleItem(XElement itemNode)
		{
			var titleNode = itemNode.Element("title");
			var datePublishedNode = itemNode.Element("pubDate");
			var authorNode = itemNode.Element("author");
			...
			Rss092FeedItem item = CreateRss092FeedItem();
